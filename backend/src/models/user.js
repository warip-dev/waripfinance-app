...
class User {
    ...
    static async updateBtcAddress(id, btcAddress) {
        await pool.execute(
            'UPDATE users SET btc_address = ? WHERE id = ?',
            [btcAddress, id]
        );
        const [rows] = await pool.execute(
            'SELECT id, btc_address FROM users WHERE id = ?',
            [id]
        );
        return rows[0];
    }

    static async updateEthAddress(id, ethAddress) {
        await pool.execute(
            'UPDATE users SET eth_address = ? WHERE id = ?',
            [ethAddress, id]
        );
        const [rows] = await pool.execute(
            'SELECT id, eth_address FROM users WHERE id = ?',
            [id]
        );
        return rows[0];
    }
